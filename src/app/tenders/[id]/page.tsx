...
const Tender: React.FC<Props> = async ({ params }) => {
  ...
  return (
    <div className="container mx-auto">
      <div className="my-15 w-full flex flex-col gap-5">
        <Link
          href="/tenders"
          className="font-bold text-primary flex items-center gap-2.5 mb-3.5"
        >
          <BackArrow /> {TEXTS.tenders.backToTenders}
        </Link>
        <div className="relative bg-[#F7F7FC] border border-[#D6D8E7] rounded-[8px] p-2.5 sm:p-4">
          <ProfileInfo
        ...
        <div className="flex flex-col gap-4 border border-[#D6D8E7] rounded-md p-3.5">
          <div className="w-full flex justify-between items-center rounded-md h-[50px] bg-[#EFF0F6] text-[#14142B] px-3.5">
            <div className="flex items-center gap-2.5">
              <p className="font-bold">{tenderDataSsr.result.name}</p>
              <p className="font-light text-[14px]">
                {TEXTS.tenders.region}: {tenderDataSsr.result.region}
              </p>
            </div>
            <p className="text-[14px]">{tenderDataSsr.result.finishes_at}</p>
          </div>
